...
async function liveMcpCheck() {
  const [
    ...
    fetchText(MCP_REVENUE_CONVERSION_QUEUE_JSON_URL),
    fetchText(MCP_REVENUE_CONVERSION_QUEUE_MARKDOWN_URL),
    fetchText(MCP_REVENUE_CONVERSION_QUEUE_HTML_URL),
  ...
  const sourceActivationHostIntentRowsRequireExternalOk = ["claude_remote_mcp", "codex_remote_mcp", "glama_connector", "mcp_so"].every((source) =>
    sourceActivationQueue?.queue?.some(
      (row) =>
        row.source === source &&
        row.current_counts?.first_run_actions > 0 &&
        row.current_counts?.mcp_tool_calls === 0 &&
        row.external_activation_required === true &&
  ...
  return check(
    ...
    health?.version === EXPECTED_VERSION &&
      ...
      agentCapture?.release === "PACKRIFT-ALL-AGENT-CAPTURE-R24" &&
      ...
      revenueConversionQueue?.source_queue_release === "PACKRIFT-MCP-SOURCE-ACTIVATION-QUEUE-R24" &&
      ...
      directorySubmitActions?.release === "PACKRIFT-MCP-DIRECTORY-SUBMIT-ACTIONS-R41" &&
      ...
      directoryUpdateCline?.release === "PACKRIFT-MCP-DIRECTORY-UPDATE-CARD-R11" &&
      ...
      agentCaptureOutreach?.release === "PACKRIFT-AGENT-CAPTURE-OUTREACH-R22" &&
      ...
      agentCaptureOutreach?.directory_submit_actions?.release === "PACKRIFT-MCP-DIRECTORY-SUBMIT-ACTIONS-R41" &&
