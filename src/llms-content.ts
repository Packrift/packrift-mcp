...
- [Tracked MCP install template](https://mcp.packrift.com/r/install/generic/codex): use \`https://mcp.packrift.com/r/install/{source}/{target}\` for target-specific install commands and install-intent attribution
- [Tracked MCP first-run template](https://mcp.packrift.com/r/run/generic/codex): use \`https://mcp.packrift.com/r/run/{source}/{target}\` for first useful MCP calls, cart-handoff proof, and source-specific activation checks
...
- [MCP agent host rollout](https://mcp.packrift.com/ai/mcp-agent-host-rollout.json): source-aware install, first-run, activation, order-handoff, and eval links for recognizable agent hosts. Flat import exports: [JSONL](https://mcp.packrift.com/ai/mcp-agent-host-rollout-tasks.jsonl) and [CSV](https://mcp.packrift.com/ai/mcp-agent-host-rollout-tasks.csv)
...
- [MCP GA4 funnel proof](https://mcp.packrift.com/ai/mcp-ga4-funnel-proof.json): public aggregate visitor, cart, revenue, and proof-gate status for the MCP channel
- [MCP source activation queue](https://mcp.packrift.com/ai/mcp-source-activation-queue.json): ranked source-specific queue for turning directory and marketplace interest into real MCP tool calls, measured cart URLs, and source-attributed proof
- [MCP revenue conversion queue](https://mcp.packrift.com/ai/mcp-revenue-conversion-queue.json): mature source queue for buyer/reviewer checkout follow-through after real MCP tool-call and qualified cart-landing proof
- [MCP buyer order handoffs](https://mcp.packrift.com/ai/mcp-buyer-order-handoffs.json): thin buyer/reviewer hub for mature source-preserving order handoffs; it links existing MCP /r/order and /r/cart routes and does not place orders. Flat checkout task exports: [JSONL](https://mcp.packrift.com/ai/mcp-buyer-order-handoffs-tasks.jsonl) and [CSV](https://mcp.packrift.com/ai/mcp-buyer-order-handoffs-tasks.csv)
- [MCP source activation sitemap](https://mcp.packrift.com/ai/mcp-source-activation-sitemap.xml): crawl map of source-specific start, config, install, first-run, eval-pack, directory-update, and reviewer-activation URLs
- [MCP activation experiment board](https://mcp.packrift.com/ai/mcp-activation-experiments.json): testable source hypotheses, tracked install/run URLs, copy-ready external requests, and expected snapshot deltas for MCP distribution work
- [MCP external activation brief](https://mcp.packrift.com/ai/mcp-external-activation-brief.json): compact real-host run brief with the selected source-aware tasks needed to move external MCP tool calls toward the material usage gate; flat selected-run exports are [JSONL](https://mcp.packrift.com/ai/mcp-external-activation-brief-tasks.jsonl) and [CSV](https://mcp.packrift.com/ai/mcp-external-activation-brief-tasks.csv)
...
- [Tracked reviewer activation template](https://mcp.packrift.com/r/activate/generic?format=html): use \`https://mcp.packrift.com/r/activate/{source}?format=html\` for browser review flows and \`https://mcp.packrift.com/r/activate/{source}?format=sh\` for shell-based review runs
- [Tracked buyer/reviewer order handoff template](https://mcp.packrift.com/r/order/generic?format=html): use \`https://mcp.packrift.com/r/order/{source}?format=html\` after real MCP tool-call and measured cart proof when a buyer or reviewer is ready to evaluate checkout; this route preserves source attribution and does not place an order
...
- [MCP eval pack](https://mcp.packrift.com/ai/mcp-eval-pack.json): host acceptance-test cases, source-aware install/run URLs, expected assertions, and reporting fields for real Packrift MCP installs
